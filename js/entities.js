...
inherit(Enemy,Element) ;                    // Enemy is a subclass of Element

function Enemy(sprite) {                		//Enemy constructor function. Inherit basic
    Element.call(this, sprite);           		//properties from Actor and create a random speed
    this.scale = 0.8;
    this.speed = (Math.floor(Math.random() * 30) + playground.numRows) / 10;
}
...
Enemy.prototype.update = function update(dt) {  	//moves enemy across the playground and re-spawns it
    ...
    if (this.position[0] > playground.numCols) {
      ...
      this.position[1] += 1 ;                        //   go to next row
      if (game.playground.layout[this.position[1]*playground.numCols + this.position[0]+1] ==="g") {
          this.position[1] = getRandomInt(1,4);                      //   go back to row 1
      } 
    }
      
};

inherit(Collectable,Element) ;           // subclass Collectable class

function Collectable(sprite) {            //Collectable constructor. Inherit basic
    Element.call(this, sprite);           //properties from Element and overwrites scale property.
    this.scale = 0.35;
}


inherit(Player,Element) ;                // subclass Player class
...
Player.prototype.update = function() {
    ...
    if (game.playground.layout[layout_index] !== "w" && game.playground.layout[layout_index] !== "k") { //checking for not win ( water or key )
        if (game.playground.layout[layout_index] !== "r")  {                  // checking for  rock 
        ...
        if (game.playground.layout[layout_index] === "k") {     // win 150 scores if new position is a key
            this.score += 150 ;
        }  else this.score += 50;                               //else win 50 scores ( water)

        if (this.score < game.level_score){                     // reset the player position  
...
Game.prototype.newLevel = function() {                              //This function holds the logic of the game and create enemies  
    ...
    // the last enemy Object is randomly placed 
    /*
    var enemy = new Enemy(images["bug"]);  
    this.allEnemies[enemies].position = [0,getRandomInt(1,4)];
    */
